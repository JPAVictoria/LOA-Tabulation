...
export async function GET() {
  try {
    ...
    // Get all judges assigned to HIPHOP competition
    const assignedJudges = await prisma.user.findMany({
      where: {
        assignedCompetition: 'HIPHOP',
        role: 'JUDGE',
        deleted: false
      },
      select: {
    ...
    const candidatesWithStatus = candidates.map((candidate) => {
      ...
      // Calculate average score for HIPHOP (single category with weighted criteria)
    ...
    console.error('Error fetching hiphop candidates:', error)
...
export async function POST(request) {
  try {
    ...
    // Verify judge exists and is assigned to HIPHOP
    ...
    // Verify candidate exists and is in HIPHOP competition
    ...
    // Validate all criteria exist and belong to HIPHOP
